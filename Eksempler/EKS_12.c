...
int main ()  {
    int   i, j;             //  STERK sedvane for navn på løkke-teller
                            //    ('i', 'j', 'k', .....)
    int   sum, valgtTall;   // Brukes ofte i for-løkker som tellere
    int   tall[MAX];
    char  tekst[STRLEN];
    ...
    for (i = 1;  i <= MAX;  i++)                                     //  Bedre, "i = 1; for hver i <= 10, + på 1", i øker altså med 1 inkrement
        printf("Bla  ");                                             // Printer da bla, i antall ganger   
    ...
    printf("\n\n'i' i tredje - baklengs:\n");           // Går baklengs, setter i = 10 i stedet, og fortsetter så lenge i er større eller lik 3
    for (i = MAX;  i >= 3;  i--)                        // Trekker da fra 1 (-1) for hvert inkrement 
        printf("\t%i:  %i\n", i, i*i*i);
    ...
    for (i = 0;  i < MAX;  i++)  {                          //  BLOKK !!!    Når en løkke inneholder mer enn en operasjon, settes {}
    ...
    for (i = 0;  i < MAX/2;  i++)  {                    // Se "i < MAX/2", halve array fra 0-9, 10/2 = 5
...
// ***************************************************************************
    ...
    printf("\n\nAlfabetet:  ");
    for (i = 0;  i < 26;  i++) {            // husk at 'TEKST' er et CHAR-array, og tall blir omgjort til bokstaver. ASCII-alfabet
...
// ***************************************************************************
    ...
    srand(time(0));                         // finner tilfeldige tall
    for (i = 0;  i < MAX;  i++)
        tall[i] = (rand() % 40) + 1;        // +1 fordi maskinen teller fra 0, derav blir det oddetall 39 når % med 40
    for (i = 0;  i < MAX;  i++)  {
        for (j = 1;  j <= tall[i];  j++)
            printf("*");
        printf("\n");
